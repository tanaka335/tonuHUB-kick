...
if not success or not Rayfield then
    warn("Rayfieldとかいう読み込みに失敗したお")
    return
end

local Window = Rayfield:CreateWindow({
    Name = "⚡tonu HUB Premium 💀",
    LoadingTitle = "tonu HUB",
...
MainTab:CreateButton({
    Name = " ラグオールキック開始 だお (20秒)",
    Callback = function()
        if lineLagEnabled then
            Rayfield:Notify({
                Title = "通知だよーん",
                Content = "すでにラグ処理してるけど？",
                Duration = 2,
            })
            return
        end
